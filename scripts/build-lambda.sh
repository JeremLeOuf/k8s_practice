...
# Build budget tracker functions
for func in add-transaction get-balance send-alert; do
    START_TIME=$(date +%s)
    echo -e "${BLUE}Building budget-tracker/$func...${NC}"
    cd $func
    ...
    END_TIME=$(date +%s)
    ELAPSED=$((END_TIME - START_TIME))
    echo -e "${GREEN}✅ budget-tracker/$func packaged (${SIZE}) in ${ELAPSED}s${NC}"
    cd ..
done
...
echo ""
echo "Main Lambda functions:"
du -ch lambda-functions/*/function.zip | tail -1
echo "Budget Tracker Lambda functions:"
du -ch budget-tracker/lambda-functions/*/function.zip | tail -1
